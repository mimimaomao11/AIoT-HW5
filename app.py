...
# 預設範例文章
DEFAULT_TEXT = """
生成式人工智慧技術在當代社會中展現出巨大的潛力。這些模型透過複雜的深度學習演算法，能夠在極短的時間內產生結構完整的長篇文檔。此類文本的特點是邏輯鏈條高度連貫、用詞精確且語氣始終保持客觀中立。由於模型在訓練過程中偏好選擇高機率的詞彙序列，這導致其輸出在統計上具有較低的困惑度與較小的句子節奏波動性。因此，若要將此類文本視為人類原創，則需要進行額外的風格修飾與語義調整，以模擬人類寫作中固有的不規則性與情緒起伏。
"""
...
def simulate_text_analysis(text_input):
    ...
    for i, sentence in enumerate(sentences):
        ...
        # 最終判斷：從 > 1.5 改為 > 0.5
        # 只要 AI 傾向得分略高於 Human 傾向得分，即判為 AI。
...
if st.button("開始分析", use_container_width=True, type="primary"):
    if len(text_input) < 50:
        st.warning("文章內容太短，請輸入至少 50 字的文本進行分析。")
    else:
